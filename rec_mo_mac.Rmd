...
```{r}
DimPlot(sample, reduction = 'umap', label = TRUE, repel = TRUE, label.size = 2.5, group.by = "SCT_snn_res.0.3", cols = paletteDiscrete(unique(sample$SCT_snn_res.0.3), set = "stallion"))
```
...
```{r}
DimPlot(recipient, reduction = 'umap', label = TRUE, repel = TRUE, label.size = 2.5, group.by = "SCT_snn_res.0.3", cols = paletteDiscrete(unique(recipient$SCT_snn_res.0.3), set = "stallion"))
```

```{r}
Idents(recipient) <- "SCT_snn_res.0.3"
recipient_neutrophils <- subset(recipient, idents = c("1", "4", "7"))
```
...
```{r}
DimPlot(recipient_neutrophils, reduction = 'umap', label = TRUE, repel = TRUE, label.size = 2.5, group.by = "SCT_snn_res.0.5", cols = paletteDiscrete(unique(recipient_neutrophils$SCT_snn_res.0.5), set = "stallion"), ncol=2)
```


```{r}
Idents(recipient_neutrophils) <- "SCT_snn_res.0.5"
recipient_neutrophils <- subset(recipient_neutrophils, idents = c("0", "2", "3"))
```
...
write.csv(rna.markers, file ="./SCT_snn_res.0.3.csv", quote = FALSE)
```

```{r}
Idents(recipient_neutrophils) <- "SCT_snn_res.0.5"
DotPlot(recipient_neutrophils, features = c("Ccr2", "Plac8", "Ace", "Ear2", "H2-Eb1", "H2-Aa"), group.by = "SCT_snn_res.0.5") + RotatedAxis()
```
...
DefaultAssay(recipient_neutrophils) <- 'SCT'
rna.markers <- FindAllMarkers(recipient_neutrophils, only.pos = TRUE, min.pct = 0.1, logfc.threshold = 0.25)
write.csv(rna.markers, file ="./DE_WT_vs_KO_recipient_monocytes_macs.csv", quote = FALSE)
```
...
FeaturePlot(object=recipient_neutrophils, features = "Inflammation",pt.size=1, reduction = 'umap', ncol = 2) + scale_color_gradientn(colors=c("blue","turquoise2","yellow","red","red4"), oob=scales::squish, limits=c(0,1))
```
...
```{r}
d <- read.csv("./DE_WT_vs_KO_recipient_monocytes_macs.csv")
...
```{r}
VlnPlot(recipient_neutrophils, features = "Ccl3", group.by = "condition")
VlnPlot(recipient_neutrophils, features = "Ccl4", group.by = "condition")
```

```{r}
saveRDS(recipient_neutrophils, "recipient_monocytes_macrophages.rds")
```

```{r}
recipient_neutrophils <- readRDS("recipient_monocytes_macrophages.rds")
```
